...
macro_indicators_interpolated <- macro_indicators |> 
  ...
  mutate(
    female_lfp_rate = approx(time, female_lfp_rate, xout = time, rule = 1)$y,
    male_lfp_rate = approx(time, male_lfp_rate, xout = time, rule = 1)$y,
    relative_lfp = female_lfp_rate/male_lfp_rate,
    
    educ_unemployment_advantage = if(sum(!is.na(educ_unemployment_advantage)) >= 2) {
      approx(time, educ_unemployment_advantage, xout = time, rule = 1)$y
    } else {
      educ_unemployment_advantage
    },
    
    educ_earnings_advantage = if(sum(!is.na(educ_earnings_advantage)) >= 2) {
      approx(time, educ_earnings_advantage, xout = time, rule = 1)$y
    } else {
      educ_earnings_advantage
    },
